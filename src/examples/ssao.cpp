...
public:
    ...
    void init(VulkanState& vulkanState, int32_t width, int32_t height, uint32_t maxFramesInFlight) {
        ...
        if (vkCreateSampler(vulkanState.device, &samplerInfo, nullptr, &colorSampler) != VK_SUCCESS) {
        ...
        renderPass.createCustom(vulkanState.device, vulkanState.swapchain, [&] {
            ...
            colorImageView = colorImage.createView(VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT, vulkanState.device);

        }, [&](std::vector<VkImageView> &attachments, VkImageView imageView) {
            attachments.push_back(colorImageView);
        });

        finalRenderPass.create(vulkanState.physicalDevice, vulkanState.device, vulkanState.allocator, vulkanState.swapchain, true, false);
